# -*- coding: utf-8 -*-
"""
Created on Thu Nov 12 15:11:09 2020
...
base1.target.value_counts()
base1.target.value_counts(normalize=True)
...
df = pd.read_csv('df_after_parse_json_format.csv')
df.date=pd.to_datetime(df.date, format='%Y-%m-%d')
...
df.groupby('year_month')['article'].nunique()
df['article'].nunique()
...
feature_last_1m.info()
feature_last_1m.describe(include = 'all')
...
#%%% Base1: 201804, Base2: 201805, Base3: 201806, 
...
base_all.year_month.value_counts()

base_all.to_csv('base_all_v2.csv')

base_all.info()
base_all.year_month.value_counts()
base_all.shape

base_all['target'].value_counts()
...
model = XGBClassifier(gamma =  0.01,min_child_weight=100)
...
gsearch1 = GridSearchCV(estimator = XGBClassifier(
 ...
 param_grid = params, scoring='roc_auc',n_jobs=4, cv=2,verbose=10)
...
#predictions =y_pred
#predictions = [predictions[i][1] for i in range(predictions.shape[0])]
#
...
output_train['rank'] = output_train['quantile'].apply(lambda a : quantile_dev(a,cutoff))

res = output_train.groupby(['rank'])['quantile'].agg([('mean',np.mean),('sum',np.sum), ('count','count')]).reset_index()    

report = pd.DataFrame({'actual':y_oot,'predict':model.predict_proba(X_oot)[:,1]})
...
report = pd.DataFrame({'actual':y_test,'predict':model.predict_proba(X_test)[:,1]})
report['rank'] = report['predict'].apply(lambda a : quantile_dev(a,cutoff))
res = report.groupby(['rank'])['actual', 'predict'].agg([('mean',np.mean),('sum',np.sum), ('count','count')]).reset_index()

report = pd.DataFrame({'actual':y_train,'predict':model.predict_proba(X_train)[:,1]})
report['rank'] = report['predict'].apply(lambda a : quantile_dev(a,cutoff))
res = report.groupby(['rank'])['actual', 'predict'].agg([('mean',np.mean),('sum',np.sum), ('count','count')]).reset_index()
...
svc = make_pipeline(StandardScaler(), SVC(kernel="rbf",probability=True,random_state=seed))
...
adb.fit(X_train.fillna(1.1e-20), y_train)

fpr, tpr, thresholds = metrics.roc_curve(y_train, adb.predict_proba(X_train.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1



fpr, tpr, thresholds = metrics.roc_curve(y_test, adb.predict_proba(X_test.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1

fpr, tpr, thresholds = metrics.roc_curve(y_oot, adb.predict_proba(X_oot.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1


gbc = GradientBoostingClassifier()
gbc.fit(X_train.fillna(1.1e-20), y_train)

fpr, tpr, thresholds = metrics.roc_curve(y_train, gbc.predict_proba(X_train.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1



fpr, tpr, thresholds = metrics.roc_curve(y_test, gbc.predict_proba(X_test.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1


fpr, tpr, thresholds = metrics.roc_curve(y_oot, gbc.predict_proba(X_oot.fillna(1.1e-20))[:,1])
2*metrics.auc(fpr, tpr)-1
